 ...
 * 版本：v0.0.1
 * 时间：2020/9/12
 * 功能：将用户网站数据生成旧版迁移数据文件，然后用户手动上传备份数据，然后参照Excel文件创建相关文件夹
 *
 */
...
//转换旧数组
var old_link_array = []; //icon先收集，然后按每页18个分拆，考虑文件夹内容

//整理过后的旧数组
var old_link_page_array = []; //分页整理，每18个分页一次，遇到文件夹开始，开始分页，遇到文件夹结束，开始分页

//执行入口
function inf_append_html_main() {
    //0. 先导出Excel 确保数据安全，以及后续步骤创建文件夹分组
    //1. infinity Pro 获取用户网站数据
    //2. 根据用户网站数据，生成旧版本备份文件，并提示用户 手动根据旧数据迁移

    var new_user_icons = localStorage.getItem("infinity-icons");
    var new_user_icons_pages = JSON.parse(new_user_icons);
...
function oldLink_to_pageLink() {
    ...
    for (var i = 0; i < old_link_array.length; i++) {
        ...
        if (countIndex == 18 || linkModel.showText == "夹开始") {
            if (countIndex == 0) {
                //刚刚翻页，不用翻页
